...
class ExcelToPDFTool:
    def __init__(self, parent_frame, file_list=None):
        ...
        # 创建界面
        self.create_excel_to_pdf_interface()
    
    def create_excel_to_pdf_interface(self):
        """创建Excel转PDF工具界面"""
        # 清除父框架中的所有内容
        ...
        # 标题
        title_label = ttk.Label(self.parent, text="Excel转PDF工具", font=('Arial', 16, 'bold'))
        title_label.pack(pady=(10, 5))
        
        # 描述
        desc_label = ttk.Label(self.parent, text="将Excel表格转换为PDF格式", 
                              font=('Arial', 12))
        ...
        add_file_btn = ttk.Button(file_buttons_frame, text="添加Excel文件", 
        ...
        self.include_all_sheets = tk.BooleanVar(value=True)
        all_sheets_check = ttk.Checkbutton(options_frame, text="包含所有工作表", 
                                         variable=self.include_all_sheets)
        all_sheets_check.pack(anchor=tk.W, pady=2)
        
        self.include_gridlines = tk.BooleanVar(value=True)
        gridlines_check = ttk.Checkbutton(options_frame, text="包含网格线", 
                                         variable=self.include_gridlines)
        gridlines_check.pack(anchor=tk.W, pady=2)
        
        self.include_headers = tk.BooleanVar(value=True)
        headers_check = ttk.Checkbutton(options_frame, text="包含列标题", 
                                      variable=self.include_headers)
        headers_check.pack(anchor=tk.W, pady=2)
    ...
    def add_excel_files(self):
        ...
        if files:
            messagebox.showinfo("成功", f"已添加 {len(files)} 个Excel文件")
    ...
    def reset_tool(self):
        ...
        if messagebox.askyesno("确认", "确定要重置所有设置吗？"):
            ...
            self.file_listbox.delete(0, tk.END)
            self.output_dir.set("")
            self.include_all_sheets.set(True)
            self.include_gridlines.set(True)
            self.include_headers.set(True)
    
    def start_convert(self):
        """开始转换Excel文件到PDF"""
        if not self.selected_files:
            messagebox.showwarning("警告", "请先添加Excel文件")
            return
        ...
        try:
            # 执行转换
            self.convert_excel_to_pdf(output_dir)
            messagebox.showinfo("成功", f"Excel文件已成功转换为PDF，保存到:\n{output_dir}")
            
        except Exception as e:
            messagebox.showerror("错误", f"转换Excel文件到PDF时出错: {str(e)}")
    
    def convert_excel_to_pdf(self, output_dir):
        """将Excel文件转换为PDF的核心功能"""
        # 这里需要实现Excel转PDF的核心功能
        # 由于openpyxl库只能读取和编辑Excel文档，不能直接转换为PDF
        # 我们需要使用其他方法，比如调用Microsoft Excel的COM接口或LibreOffice命令行
...
# 独立测试函数
def test_excel_to_pdf_tool():
    """独立测试Excel转PDF工具"""
    root = tk.Tk()
    root.title("Excel转PDF工具测试")
    root.geometry("800x600")
    
    frame = ttk.Frame(root)
    frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    
    excel_to_pdf_tool = ExcelToPDFTool(frame)
    
    root.mainloop()

if __name__ == "__main__":
    test_excel_to_pdf_tool()